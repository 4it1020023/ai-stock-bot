...
def generate_chart(df: pd.DataFrame, symbol: str, n_candles: int = 60) -> str:
    ...
    fig.text(0.99, 0.01, f"Generated: {now_str}  |  For reference only",
...
def webhook():
    ...
    for event in data.get("events", []):
        ...
        if not symbol:
            t = text.upper()
            # 加密貨幣：自動補 USDT
            if t in ["BTC","ETH","BNB","SOL","XRP","DOGE","ADA","AVAX",
            ...
            # 直接當成美股代號嘗試
                ...
                symbol = t  # 例如 HTC, TSLA, META 等

        reply_text(reply_token, f"⏳ Analyzing {symbol}...")
        try:
            import yfinance as yf
            ...
            if is_crypto:
                ...
                if df.empty or len(df) < 5:
                    df = get_stock_df(symbol + ".TW")
                    symbol = symbol + ".TW"

            if df.empty or len(df) < 5:
                push_text(f"⚠️ Cannot find data for [{symbol}]\nPlease check the symbol.\nExamples: TSLA, 2330, BTC, NVDA", user_id)
            else:
            ...
            push_text(f"⚠️ Query failed [{symbol}]: {str(e)[:100]}\n\nTry: TSLA / 2330 / BTC / NVDA", user_id)
