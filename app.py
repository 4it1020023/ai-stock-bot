...
def format_analysis(symbol: str, result: dict) -> str:
    ...
    lines += result["info"]
    lines.append("─────────────")
    lines.append("📋 綜合評估：" + result.get("verdict", "—"))
    lines.append("⭐ 訊號強度：" + result.get("stars", "—"))
    lines.append("📌 近期壓力：" + str(result.get("resistance", "—")))
    lines.append("📌 近期支撐：" + str(result.get("support", "—")))
    lines.append("─────────────")
    lines.append("⚠️ 以上僅供參考，非投資建議")
    return "\n".join(lines)
