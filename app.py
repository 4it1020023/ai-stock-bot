...
def analyze(df: pd.DataFrame) -> dict:
    ...
    if pct_change >= SPIKE_THRESHOLD:
        important.append("[!!] Spike UP " + f"{pct_change:.1f}" + "%!")
    elif pct_change <= -SPIKE_THRESHOLD:
        important.append("[!!] Spike DOWN " + f"{pct_change:.1f}" + "%!")

    # ── RSI ──
    rsi = latest["rsi"]
    if rsi > RSI_OVERBOUGHT:
        important.append("[!] RSI Overbought (" + f"{rsi:.1f}" + "), watch for pullback")
    elif rsi < RSI_OVERSOLD:
        important.append("[!] RSI Oversold (" + f"{rsi:.1f}" + "), watch for bounce")

    # ── MACD 交叉 ──
    if prev["macd"] < prev["macd_signal"] and latest["macd"] > latest["macd_signal"]:
        important.append("[+] MACD Golden Cross (Bullish)")
    elif prev["macd"] > prev["macd_signal"] and latest["macd"] < latest["macd_signal"]:
        important.append("[-] MACD Death Cross (Bearish)")

    # ── 布林通道 ──
    if latest["close"] > latest["bb_upper"]:
        important.append("[^] Price broke BB Upper (watch pullback)")
    elif latest["close"] < latest["bb_lower"]:
        important.append("[v] Price broke BB Lower (watch bounce)")

    # ── 均線 ──
    trend = "[UP] MA Bullish trend" if latest["ma20"] > latest["ma60"] else "[DN] MA Bearish trend"
    info.append(trend)
...
def format_analysis(symbol: str, result: dict) -> str:
    ...
    if result["important"]:
        ...
        lines.append("[OK] No major signals")
    lines += result["info"]
    lines.append("─────────────")
...
def webhook():
    ...
    for event in data.get("events", []):
        ...
        reply_text(reply_token, "Analyzing " + symbol + "...")
        try:
            ...
            if df is None or df.empty or len(df) < 5:
                push_text("[Warning] Cannot find: " + symbol + ". Examples: TSLA / 2330 / BTC / NVDA", user_id)
            else:
            ...
            push_text("[Error] " + symbol + ": " + str(e)[:100] + " | Try: TSLA/2330/BTC/NVDA", user_id)
